...
// This allows us to solve the problem efficiently using various dynamic programming techniques such as recursion, 
// memoization, tabulation, or space optimization.
...
// Explanation: The space-optimized solution further reduces the space complexity by using only two variables (prev and curr) 
// instead of an entire DP table. It initializes prev and curr to 1 since there is only one way to 
// reach the base cases (0 and 1 steps). Then, in each iteration, it updates prev and curr by shifting their values. 
